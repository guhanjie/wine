(function($) {
	...
	$.get("/wine/admin/item/all", function(data){
		...
		//'<span class="del-item weui-badge" style="position: absolute;top: 0.5em;right: 0.5em;">X</span>'+
		'</a>';
...
//			      '        <ul class="weui-uploader__files">'
    ...
    function initialImgUploader(uploader) {
	    ...
	    weui.uploader('#'+uploader.type+'_imgs-uploader', {
	        ...
	        onBeforeSend: function(data, headers){
	            ...
	            $.extend(data, { imgType: uploader.type }); // 可以扩展此对象来控制上传参数
    ...
    // 编辑商品信息
    $('#item-list').on("click", '.item.weui-grid', function(event) {
	var $item = $(this);
	weui.actionSheet([
	        {
	            label: '编辑',
	        	...
	        	if(item.icon) {
	        	    iconUploader.uploadList[0] = {'content':item.icon, 'id':item.icon};
	        	    $('#iconImgsFiles').empty();
	        	    $('#iconImgsFiles').append('<li class="weui-uploader__file" data-id="'+item.icon+'" style="background-image: url(&quot;/wine/resources/'+item.icon+'&quot;);"></li>');
	        	}
	        	...
	        	if(item.titleImgs) {
	        		...
	        		titleUploader.uploadList[i] = {'content': e, 'id':e};
	        		$('#titleImgsFiles').append('<li class="weui-uploader__file" data-id="'+e+'" style="background-image: url(&quot;/wine/resources/'+e+'&quot;);"></li>');
	        	    });
	        	}
	        	...
	        	if(item.detailImgs) {
	        		...
	        		detailUploader.uploadList[i] = {'content': e, 'id':e};
	        		$('#detailImgsFiles').append('<li class="weui-uploader__file" data-id="'+e+'" style="background-image: url(&quot;/wine/resources/'+e+'&quot;);"></li>');
	        	    });
	        	}
	        	$('#item-list').hide();
	        	$('#item-form').show();
...
//		setTimeout(function() {loading.hide();}, 1500);
    ...
    $('.back-item-list').on('click', function() {
	$('#item-form').hide();
	$('#item-list').show();
    });
